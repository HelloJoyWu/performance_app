...
interface motivationScreenProps {
  navigation: StackNavigationProp<RootStackParamList, 'MotivationScreen'>;
  route: RouteProp<RootStackParamList, 'MotivationScreen'>;
}
...
const MotivationScreen: React.FC<motivationScreenProps> = ({
  ...
  const agentArray: any[] = ['all', 'cq9', '168', 'cl'];
  ...
  const fetchLatest = useCallback(async () => {
    try {
      ...
      console.log('Loading latest motivation performane success');
      setLatestTableData([resp1.data[0], resp2.data[0], resp3.data[0]]);
    } catch (error: any) {
      const alertMsg = error.response.config.url
        ? `[${error.response.status}] ${error.response.data.message}`
        : `${error}`;
      console.error('Fetch latest motivstion failed', alertMsg);
    }
  }, []);
      ...
      return (
        <View key={obj.name + '_lineChart'} style={styles.lineChartBox}>
          <Pressable
            ...
            <View style={styles.tableRowBigBox}>
              <Image
                source={require('../../../res/gameType/motivation_bar.png')}
